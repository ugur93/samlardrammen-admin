...
import { CreateUpdateUserFormFields, mapToFormValues } from '../types/formTypes';
          ...
          <TableBody>
            {data?.map((personDetails) => (
              <TableRow key={personDetails.person.id}>
                <TableCell><Link href={`/${personDetails.person.id}`}>{personDetails.person.firstname} {personDetails.person.lastname}</Link></TableCell>
                <TableCell>{personDetails.person.birthdate ? new Date(personDetails.person.birthdate).toLocaleDateString() : "-"}</TableCell>
                <TableCell>{personDetails.person.email}</TableCell>
                <TableCell><IconButton onClick={() => setCreateOrEdit(personDetails)}><EditIcon /></IconButton></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </TableContainer>
      {createOrEdit != false && <CreateOrEditUserDialog open person={typeof createOrEdit == "object" ? createOrEdit : undefined} onClose={() => setCreateOrEdit(false)} refetch={() => null} />}
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        ...
        <FormControlLabel
          control={
            <Checkbox checked={paymentStatus} onChange={(e) => setPaymentStatus(e.target.checked)} />
          }
          label="Payment Made"
        />
      </DialogContent>
...
const CreateOrEditUserDialog: React.FC<CreateUserDialogProps> = ({ open, onClose, person }) => {
  ...
  const {
    ...
    getValues,
    watch,
  } = useForm<CreateUpdateUserFormFields>({
    defaultValues: mapToFormValues(person),
  });

  const onSubmit: SubmitHandler<CreateUpdateUserFormFields> = async (data) => {
