...
export const AppContextProvider: React.FC<AppContextProviderProps> = ({ children }) => {
    ...
    return (
        <AppContext.Provider value={{ user: loggedInUser, roles: loggedInUser?.roles ?? [], isAdmin: loggedInUser?.roles.includes("admin") ?? false, pages: getPages(loggedInUser?.roles ?? []) }}>
            {children}
        </AppContext.Provider>
    );
};
